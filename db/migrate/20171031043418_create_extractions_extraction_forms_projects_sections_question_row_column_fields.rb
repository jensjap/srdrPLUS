class CreateExtractionsExtractionFormsProjectsSectionsQuestionRowColumnFields < ActiveRecord::Migration[5.0]
  def change
    ...
    add_index :eefps_qrcf, [:extractions_extraction_forms_projects_section_id, :question_row_column_field_id, :deleted_at], name: 'index_eefpsqrcf_on_eefps_id_qrcf_id_deleted_at', where: 'deleted_at IS NULL'
    add_index :eefps_qrcf, [:extractions_extraction_forms_projects_section_id, :question_row_column_field_id, :active],     name: 'index_eefpsqrcf_on_eefps_id_qrcf_id_active'
  end
end
