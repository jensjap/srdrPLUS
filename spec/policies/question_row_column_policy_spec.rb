...
describe QuestionRowColumnPolicy, type: :controller do
  ...
  subject { QuestionRowColumnPolicy.new(user, project) }

  context 'for a non_member' do
    let(:user) { non_member }
    it { should_not permit(:destroy_entire_column) }
    it { should_not permit(:answer_choices) }
  end

  context 'for leader' do
    let(:user) { leader }
    it { should permit(:destroy_entire_column) }
    it { should permit(:answer_choices) }
  end
