class ProjectReportLinksController < ApplicationController
  ...
  def order_groups_hash(hash)
    ...
    hash.each do |key, values|
      index = values.first.extraction_forms_projects_section.pos
  ...
  def get_rssm_groups(project_id, type1s)
    ...
    rssms.each do |key, values|
      values_with_efps_id = values.map do |rssm|
        { rssm:,
          efps_id: rssm.result_statistic_section.population.extractions_extraction_forms_projects_section.extraction_forms_projects_section_id }
      end
