class ExtractionsController < ApplicationController
  ...
  before_action :set_project,
                only: %i[index new create comparison_tool consolidate edit_type1_across_extractions]
  ...
  before_action :skip_policy_scope, except: %i[consolidate edit_type1_across_extractions]
  before_action :skip_authorization, only: %i[index show]

  # GET /projects/1/extractions
  # GET /projects/1/extractions.json
  def index
    @nav_buttons.push('extractions_dropdown', 'extractions', 'my_projects')
    @extractions =
      @project
      .extractions
      .unconsolidated
  ...
  def comparison_tool
    ...
    authorize(@project, policy_class: ExtractionPolicy)
    @nav_buttons.push('extractions_dropdown', 'comparison_tool', 'my_projects')
