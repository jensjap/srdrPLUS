  ...
  module V1
    class TagsController < BaseController
      ...
      def index
        ...
        combined_tags = ( lead_tags + projects_user_tags + user_tags ).uniq
        offset        = [ page_size * ( page - 1 ), combined_tags.length ].min

        @tags = combined_tags[ offset .. offset + page_size - 1 ]
        @more = if offset + @tags.length < combined_tags.length then true else false end      
      end
