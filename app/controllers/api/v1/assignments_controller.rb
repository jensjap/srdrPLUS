module Api
  module V1
    class AssignmentsController < BaseController
      before_action :set_assignment, only: %i[screen history]
      before_action :prepare_highlighting, only: %i[screen history]
      ...
      def screen
        @unlabeled_citations_projects = CitationsProject
                                        ...
                                        .includes(citation: %i[authors keywords journal])
        @unlabeled_taggings = Tagging.where(taggable_type: 'CitationsProject',
                                            taggable_id: @unlabeled_citations_projects.map do |cp|
                                                           cp.id
                                                         end,
        ...
        @past_labels = Label.last_updated(@assignment.projects_users_role, 0, params[:count])
                            .includes(
                              ...
                              citations_project: [
                                citation: %i[authors keywords journal]
                              ]
                            )
      ...
      def prepare_highlighting
        putgcs = ProjectsUsersTermGroupsColor.where(projects_user: @assignment.projects_user).includes(:terms,
                                                                                                       term_groups_color: [:color])
        @terms_dict = {}
      ...
      def highlight_terms(input_string)
        if input_string.present? and @terms_dict.size > 0
          input_string.gsub(@terms_regexp) do |match|
            '<b style="color: ' + @terms_dict[match] + ';">' + match + '</b>'
          end
        else
          input_string
        end
