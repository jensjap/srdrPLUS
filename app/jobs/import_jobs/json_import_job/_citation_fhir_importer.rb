class CitationFhirImporter
  ...
  def run
    ...
    journal_hash = {
      ...
      publication_date: @citation_json.fetch('journal', {}).fetch('journalIssue', {}).fetch('publicationDate', {}).fetch(
        'text', ''
      )
    }
  ...
  def fetch_pmid_from_identifiers
    @citation_json['identifier'].each do |_idx, identifier|
      return identifier['value'] if identifier['system'].eql?('https://www.ncbi.nlm.nih.gov/pubmed/')
    end
  ...
  def fetch_doi_from_identifiers
    @citation_json['identifier'].each do |_idx, identifier|
      return identifier['value'] if identifier['system'].eql?('https://doi.org')
    end
