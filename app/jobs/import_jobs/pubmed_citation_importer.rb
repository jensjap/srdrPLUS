...
module ImportJobs::PubmedCitationImporter
  def import_citations_from_pubmed_file(imported_file, user_id, import_id, preview = false)
    pmid_arr = imported_file.content.download.split("\n").map(&:strip).compact_blank
    import_citations_from_pubmed_array(imported_file.project, pmid_arr, user_id, import_id, preview)
  end

  def import_citations_from_pubmed_array(project, pubmed_id_array, user_id, import_id, preview)
    count = pubmed_id_array.length
    pubmed_id_array = pubmed_id_array[0..4] if preview

    preview_citations = process_pubmed_ids(project, pubmed_id_array, user_id, import_id, preview)
  ...
  def process_pubmed_ids(project, pubmed_id_array, user_id, import_id, preview)
    preview_citations = []
    ...
    unless preview
      ...
      pubmed_id_array.each do |pmid|
        ...
        CitationsProject.find_or_create_by!(project:, citation:, creator_id: user_id, import_type: 'pubmed', import_id: import_id)
      end
    end
