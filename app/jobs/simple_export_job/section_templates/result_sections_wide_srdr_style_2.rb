module SimpleExportJob::SectionTemplates::ResultSectionsWideSrdrStyle2
  def build_result_sections_wide_srdr_style_2(kq_ids = [], _print_empty_row = false)
  ...
  def _export_standard_ef_results
    ...
    @sheet_info.extractions.each do |_e_key, extraction|
      ...
      extraction[:rss_columns].each do |rss_type_id, outcomes|
        outcomes.each do |_outcome_id, populations|
          populations.each do |_population_id, rss_rows|
            rss_rows.each do |_row_id, rss_cols|
              new_row = []
  ...
  def build_data_row(rss_cols, header)
    ...
    rss_cols.each_with_index do |col, idx|
      ...
      col[1].each do |rssm|
        ...
        length_of_each_col_group = @sheet_info.data_header_hash.try(:[], :max_col).try(:[], rssm[:result_statistic_section_type_id]).try(:[], rssm[:outcome_type]).try(
          :[], :no_of_measures
        )
  ...
  def find_index_of_cell_with_value(row, value = nil)
    row.cells.each do |cell|
      ...
      elsif cell.value.match(/^Arm Name 1$|^Comparison Name 1$/)
  ...
  def _print_da_results_headers
    ...
    @ws_descriptive_statistics.add_row(
      @sheet_info.header_info +
      ['Diagnosis', 'Diagnosis Description', 'Population', 'Population Description', 'Timepoint', 'Timepoint Unit',
       'Comparison'] +
