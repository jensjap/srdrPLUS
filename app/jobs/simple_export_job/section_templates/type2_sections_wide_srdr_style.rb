module SimpleExportJob::SectionTemplates::Type2SectionsWideSrdrStyle
  def build_type2_sections_wide_srdr_style(kq_ids = [], _print_empty_row = false)
        ...
        sheet_info.extractions.each do |_key, extraction|
          ...
          eefpst1s.each do |eefpst1|
            ...
            extraction[:question_row_columns].each do |qrc|
              next unless qrc[:eefpst1_id].eql?(eefpst1.id)
              ...
              unless found
                raise "Error: Could not find header row: [Question ID: #{qrc[:question_id]}][Field ID: #{qrc[:question_row_id]}x#{qrc[:question_row_column_id]}]"
              end

              new_row[column_idx] = qrc[:eefps_qrfc_values]
              # END if qrc[:type1_id].eql?(eefpst1.id)
