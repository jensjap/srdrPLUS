...
class GsheetsExportJob < ApplicationJob
  ...
  def perform(*args)
    ...
    Axlsx::Package.new do |p|
      ...
      highlight  = p.workbook.styles.add_style bg_color: 'C7EECF', fg_color: '09600B', sz: 14,
...
class CustomExporter
  ...
  def write_extraction_rows(extraction, sheet, _highlight)
    ...
    combination_arr.each do |current_combination|
      current_row = row_prefix + get_combination_data_string(current_combination, @comparate_1_length,
                                                             @comparate_2_length)
      @col_args.each do |_, col_hash|
        ...
        col_hash['export_items'].each do |_, export_item|
          if export_item['type'] == 'Type2'
            ...
            process_results(rich_text, rss, [export_item['export_id']], arm_eefpst1, outcome_eefpst1, eefpst1r,
                            eefpst1rc, arm_comp, tp_comp)
            rich_text.add_run "\n"
          end
        end
  ...
  def create_efps_dict(inclusion_dict, efps_arr)
    # other type1s
    t1_efps_dict = {}
    t1_efps_dict[inclusion_dict['arms_efps'].id] = 0 if inclusion_dict['arms_efps']
    t1_efps_dict[inclusion_dict['outcomes_efps'].id] = 1 if inclusion_dict['outcomes_efps']
    i = 5
    ...
    q.question_rows.each_with_index do |qr, ri|
      qr.question_row_columns.each_with_index do |qrc, ci|
        ...
        record_arr = Record.where(recordable_type: 'ExtractionsExtractionFormsProjectsSectionsQuestionRowColumnField',
                                  recordable_id: eefpsqrcf_arr.map do |eefpsqrcf|
                                                   eefpsqrcf.id
                                                 end).sort { |a, b| a.recordable_id <=> b.recordable_id }

        next if record_arr.empty? or record_arr.first.name.nil?

        case qrc_type_id
        when 1
          rich_text.add_run(record_arr.first.name.to_s) if record_arr.length == 1
        when 2
        ...
        when 5
          if record_arr.length == 1
            option_names = (record_arr.first.name[2..-3].split('", "') - ['']).map do |r|
              QuestionRowColumnsQuestionRowColumnOption.find(r.to_i).name
            end
  ...
  def process_qids(rich_text, qid_arr, extraction, combination, t1_efps_dict)
    qid_arr.each do |qid|
      ...
      eefpst1 = combination[t1_efps_dict[t1_efps.id]] if t1_efps.present?
  ...
  def get_headers
    ...
    header_arr << 'Arm' if inclusion_dict['arms_efps'].present?
    header_arr << 'Outcome' if inclusion_dict['outcomes_efps'].present?
    header_arr << 'Population' if inclusion_dict['populations']
    header_arr << 'Timepoint' if inclusion_dict['timepoints']
    if inclusion_dict['arms_comparisons']
      header_arr += ((1..(@comparate_1_length + @comparate_2_length)).map.with_index { |_x, i| "Arm #{i + 1}" })
  ...
  def get_combination_data_string(combination, comp1_length, comp2_length)
    string_arr = []
    string_arr << combination[0].type1.name if combination[0].present?
    string_arr << combination[1].type1.name if combination[1].present?
    string_arr << combination[3].population_name.name if combination[3].present?
    ...
    if combination[4].present?
      if combination[4].comparate_groups.length == 0
        ...
        comp1_string_arr = combination[4].comparate_groups.first.comparates.all.map do |c|
          c.comparable_element.comparable.type1.name
        end
        comp2_string_arr = combination[4].comparate_groups.second.comparates.all.map do |c|
          c.comparable_element.comparable.type1.name
        end
        ...
        string_arr << comp1_string_arr.map.with_index do |_x, i|
          "Arm #{i + 1}"
        end.join(' and ') + ' vs. ' + comp2_string_arr.map.with_index do |_x, i|
                                        "Arm #{comp1_string_arr.length + i + 1}"
                                      end.join(' and ')
      end
    end
  ...
  def process_results(rich_text, rss, mid_arr, arm_eefpst1, _outcome_eefpst1, _eefpst1r, eefpst1rc, arm_comp, tp_comp)
    _first = true
  ...
  def get_comparate_lengths(project)
    ...
    ComparisonsResultStatisticSection.where(
        ...
        population: ExtractionsExtractionFormsProjectsSectionsType1Row.where(
          extractions_extraction_forms_projects_sections_type1: ExtractionsExtractionFormsProjectsSectionsType1.where(
            extractions_extraction_forms_projects_section: ExtractionsExtractionFormsProjectsSection.where(
              extraction: Extraction.where(project:),
              extraction_forms_projects_section: ExtractionFormsProjectsSection.where(
                section: Section.where(name: 'Outcomes')
              )
            )
          )
        )
      ...
      comp1_length = comp1.comparates.length if comp1.present? and comp1.comparates.length > comp1_length
      comp2_length = comp2.comparates.length if comp2.present? and comp2.comparates.length > comp2_length
    end
    [comp1_length, comp2_length]
  end
end
