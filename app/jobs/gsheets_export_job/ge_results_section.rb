module GsheetsExportJob::GeResultsSection
    ...
    project.extraction_forms_projects.each do |efp|
      efp.extraction_forms_projects_sections.each do |efps|
        ...
        p.workbook.add_worksheet(name: "#{efps.section.name.truncate(30)}") do |sheet|
          # For each sheet we create a SimpleExportJob::SheetInfo object.
          sheet_info = SimpleExportJob::SimpleExportJob::SheetInfo.new
          ...
          sheet_info.extractions.each do |_key, extraction|
            ...
            extraction[:rssms].each do |rssm|
              ...
              unless found
                raise "Error: Could not find header row: [Outcome ID: #{rssm[:outcome_id]}][Population ID: #{rssm[:population_id]}][RSS Type ID: #{rssm[:result_statistic_section_id]}][Row ID: #{rssm[:row_id]}][Col ID: #{rssm[:col_id]}][Measure ID: #{rssm[:measure_id]}]"
              end
        ...
        # END if efps.extraction_forms_projects_section_type_id == 3
