...
class ExtractionFormsProject < ApplicationRecord
  ...
  private
    ...
    def ensure_proper_sections
      if self.extraction_forms_project_type_id.eql?(1)
        sections_for_efp_type1

      elsif self.extraction_forms_project_type_id.eql?(2)
    ...
    def sections_for_efp_type1
      ExtractionFormsProjectsSection.with_deleted
        ...
        .where(section: [Section.where('sections.name in (?)', STANDARD_SECTIONS
      )]).each do |efps|
    ...
    def sections_for_efp_type2
      DIAGNOSTIC_TEST_SECTIONS.each do |section_name|
        ...
        if efps
          ...
          ExtractionFormsProjectsSection.create({
            extraction_forms_project: self,
            extraction_forms_projects_section_type: ['Diagnostic Tests', 'Diagnoses'].include?(section_name) ?
            ExtractionFormsProjectsSectionType.find_by(name: 'Type 1') : ['Diagnostic Test Details', 'Diagnosis Details'].include?(section_name) ?
            ExtractionFormsProjectsSectionType.find_by(name: 'Type 2') : raise('Unexpected default section'),
            section: Section.find_or_create_by(name: section_name),
            link_to_type1: ['Diagnostic Test Details'].include?(section_name) ?
            ExtractionFormsProjectsSection.find_by(extraction_forms_project: self, extraction_forms_projects_section_type: ExtractionFormsProjectsSectionType.find_by(name: 'Type 1'), section: Section.find_by(name: 'Diagnostic Tests')) : ['Diagnosis Details'].include?(section_name) ?
            ExtractionFormsProjectsSection.find_by(extraction_forms_project: self, extraction_forms_projects_section_type: ExtractionFormsProjectsSectionType.find_by(name: 'Type 1'), section: Section.find_by(name: 'Diagnoses')) : nil })
        end
      end

      self.extraction_forms_projects_sections
        .where({ extraction_forms_projects_sections: { section: [Section.where('sections.name in (?)', STANDARD_SECTIONS
      )] } }).each(&:destroy)
    end
end
