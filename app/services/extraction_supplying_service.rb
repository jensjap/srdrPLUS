class ExtractionSupplyingService
  ...
  def merge_statistics(array)
    array.map do |item|
      ...
      note_arr = [] # added this line to create an array for notes
      merged_statistic = merge_targets.reduce({}) do |result, current|
        ...
        if current.key?("note") # if current statistic has note
          note_arr += current["note"] # append it to the notes array
      ...
      unless note_arr.empty? # if notes array is not empty
        merged_statistic["note"] = note_arr # set it to the merged statistic
      end
