...
const copyToClipboardMessage = (e) => {
  ...
  $(e.target).append(
    '<span class="copied-message" style="color: green; font-size: 0.8em;"> Copied!</span>'
  );
  $(".copied-message")
    .delay(800)
    .fadeOut(400)
...
const attachOrderable = (orderable) => {
  ...
  const sendPositions = (
    orderable,
    dropConflictingDependencies = false,
    ...
    $.ajax({
      ...
      error(data) {
        ...
        return toastr.error(
          "ERROR: Cannot save new position due to a server error."
        );
      },
    });
  };
  ...
  const ensureValidOrder = (e) => {
    ...
    lsofOrderableItemsInReverse = $(e.item)
      .parent(".orderable-list")
      .find(".orderable-item")
      .get()
      .reverse();
  ...
  let _sortable = new Sortable(orderable, {
    ...
    onUpdate: (e) => {
      ...
      let { validOrder, lsofOrderingsToRemoveDependencies } =
        ensureValidOrder(e);

      if (validOrder == true) {
        sendPositions(orderable, false);
      } else {
        overwrite_dependencies = confirm(
          "The system detected a problem with dependencies due to re-ordering of the questions. Would you like to proceed regardless and have the system remove the conflicting dependencies for you?"
        );
        if (overwrite_dependencies == true) {
          sendPositions(orderable, true, lsofOrderingsToRemoveDependencies);
          toastr.warning(
            "WARNING: You have chosen to re-order despite a dependency conflict. Conflicting dependencies have been automatically removed, please check the dependency structure."
          );
        } else {
          toastr.warning(
            "WARNING: Cannot save new positions. A dependency is preventing the new ordering from being saved. Please refresh the page to update the ordering. You may remove the conflicting dependencies manually to try again."
          );
        }
      }
    },
...
let documentCode = function () {
  ...
  if (
    $(
      "body.extractions.work, body.extractions.comparison_tool, body.extractions.consolidate"
    ).length > 0
  ) {
    $("body").on("click", ".status-switch", function () {
      var $outer_form = $(this).parents("form");
      var $outer_input = $outer_form.find("select");
      var draft_id = $outer_form
        .find("option")
        .filter(function () {
          return $(this).html() == "Draft";
        })
        .val();
      var completed_id = $outer_form
        .find("option")
        .filter(function () {
          return $(this).html() == "Completed";
        })
        .val();
