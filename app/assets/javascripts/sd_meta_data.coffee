...
updateSectionFlag = (domEl) ->
  ...
  aaa = $('.sd-form')[0].action
  $.post "#{aaa}/section_update", { sd_meta_datum: "#{paramKey}": status }, (data) ->
...
initializeSwitches = ->
  if '#{@sd_meta_datum.section_flag_0}' == 'true'
    check 0, true
  if '#{@sd_meta_datum.section_flag_0}' == 'true' and $('#0-yes-no-section')[0]
    $('#0-yes-no-section')[0].checked = true
  if '#{@sd_meta_datum.section_flag_1}' == 'true'
    check 1, true
  if '#{@sd_meta_datum.section_flag_1}' == 'true' and $('#1-yes-no-section')[0]
    $('#1-yes-no-section')[0].checked = true
  if '#{@sd_meta_datum.section_flag_2}' == 'true'
    check 2, true
  if '#{@sd_meta_datum.section_flag_2}' == 'true' and $('#2-yes-no-section')[0]
    $('#2-yes-no-section')[0].checked = true
  if '#{@sd_meta_datum.section_flag_3}' == 'true'
    check 3, true
  if '#{@sd_meta_datum.section_flag_3}' == 'true' and $('#3-yes-no-section')[0]
    $('#3-yes-no-section')[0].checked = true
  if '#{@sd_meta_datum.section_flag_4}' == 'true'
    check 4, true
  if '#{@sd_meta_datum.section_flag_4}' == 'true' and $('#4-yes-no-section')[0]
    $('#4-yes-no-section')[0].checked = true
  if '#{@sd_meta_datum.section_flag_5}' == 'true'
    check 5, true
  if '#{@sd_meta_datum.section_flag_5}' == 'true' and $('#5-yes-no-section')[0]
    $('#5-yes-no-section')[0].checked = true
  if '#{@sd_meta_datum.section_flag_6}' == 'true'
    check 6, true
  if '#{@sd_meta_datum.section_flag_6}' == 'true' and $('#6-yes-no-section')[0]
    $('#6-yes-no-section')[0].checked = true
  if '#{@sd_meta_datum.section_flag_7}' == 'true'
    check 7, true
  if '#{@sd_meta_datum.section_flag_7}' == 'true' and $('#7-yes-no-section')[0]
    $('#7-yes-no-section')[0].checked = true
  return

$(document).on 'turbolinks:load', ->
  #setTimeout(function(){ initializeSwitches(); }, 500);
  initializeSwitches()
  # var form = $('#sd-meta-form');
