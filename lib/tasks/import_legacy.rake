  ...
  namespace(:import_legacy) do
    ...
    def migration_user
      if @user
        ...
        @user = User.create(email: 'migration_service_account@local.host',
                            password: ENV['SRDRPLUS_MIGRATION_SERVICE_ACCOUNT_PASSWORD'])
        if @user.valid?
          @user
        else
    ...
    def get_qrcqrco(qrc_id, option_text)
      @qrcqrco_dict[qrc_id][option_text] if @qrcqrco_dict.key? qrc_id
    end
    ...
    def queue_data_point(dp, qrcf, dp_type = nil)
      ...
      @data_points_queue[study_id][qrcf.id] ||= { data_points: [], qrcf:, question_type:,
                                                  dp_type: }
      unless @data_points_queue[study_id][qrcf.id][:data_points].include? dp
    ...
    def migrate_legacy_srdr_project(project_hash)
      ...
      studies_hash.each do |study_hash|
        ...
        primary_publications.each do |primary_publication_hash|
          ...
          citations_project = CitationsProject.create citation: citation,
    ...
    def migrate_extraction_forms_as_standard_efp(efs)
      ...
      efs.each do |ef|
        ...
        ef_sections.each do |ef_section|
          ...
          when 'arms'
            arms_efps = combined_efp.extraction_forms_projects_sections.find_or_create_by(section:,
                                                                                          extraction_forms_projects_section_type: @efps_type_1)
            set_efps 'arms', arms_efps
            adverse_events_efps.update(link_to_type1: arms_efps) if adverse_events_efps.present?
            ...
            ef_arms.each do |ef_arm|
              ...
              ExtractionFormsProjectsSectionsType1.create extraction_forms_projects_section: arms_efps, type1:
            end
          when 'outcomes'
            outcomes_efps = combined_efp.extraction_forms_projects_sections.find_or_create_by(section:,
                                                                                              extraction_forms_projects_section_type: @efps_type_1)
          ...
          when 'diagnostics'
            diagnostic_tests_efps = combined_efp.extraction_forms_projects_sections.find_or_create_by(section:,
                                                                                                      extraction_forms_projects_section_type: @efps_type_4)
            ...
            ef_diagnostic_tests.each do |ef_diagnostic_test|
              type1 = Type1.find_or_create_by name: ef_diagnostic_test['title'],
                                              description: ef_diagnostic_test['description']
          ...
          when 'quality', 'arm_details', 'outcome_details', 'quality_details', 'diagnostic_test_details', 'design', 'baselines'
            section = Section.find_or_create_by(name: ef_section['section_name'].titleize)
            t2_efps << combined_efp.extraction_forms_projects_sections.find_or_create_by(section:,
                                                                                         extraction_forms_projects_section_type: @efps_type_2)
          when 'results'
            section = Section.find_or_create_by(name: ef_section['section_name'].titleize)
            combined_efp.extraction_forms_projects_sections.find_or_create_by(section:,
                                                                              extraction_forms_projects_section_type: @efps_type_results)
          else
        ...
        t2_efps.each do |efps|
          ...
          when 'Arm Details'
            ...
            efsos.each do |efso|
              efps.update link_to_type1: arms_efps if efso['by_arm'] == 1
              efps.extraction_forms_projects_section_option.update \
                by_type1: (efso['by_arm'] == 1),
                include_total: (efso['include_total'] == 1)
            end
          ...
          when 'Outcome Details'
            ...
            efsos.each do |efso|
              efps.update link_to_type1: outcomes_efps if efso['by_outcome'] == 1
              efps.extraction_forms_projects_section_option.update \
                by_type1: (efso['by_outcome'] == 1),
                include_total: (efso['include_total'] == 1)
            end
          ...
          when 'Diagnostic Test Details'
            ...
            efsos.each do |efso|
              efps.update link_to_type1: diagnostic_tests_efps if efso['by_diagnostic_test']
              efps.extraction_forms_projects_section_option.update \
                by_type1: (efso['by_diagnostic_test'] == 1),
                include_total: (efso['include_total'] == 1)
            end
    ...
    def get_question_fields(table_root, ef_id)
      ...
      if q_ids_string.present?
        db.query "SELECT * FROM #{table_root}_fields where #{table_root}_id IN (#{q_ids_string}) ORDER BY `row_number` ASC"
      else
        []
      end
    end
    ...
    def get_qdf_dropdown_options(quality_dimension)
      ...
      if defined?(dimensions_file) && !dimensions_file.nil?
        ...
        dimensions_file.each do |section|
          next unless defined?(section['dimensions']) && !section['dimensions'].nil?

          section['dimensions'].each do |dimension|
            next if finished
            # test if dimension['question'] equals first part of @qd_text
            unless defined?(dimension['question']) && !dimension['question'].nil? && qd_text.starts_with?(dimension['question'])
              next
            end
            ...
            finished = true
            # end compare question and yml question
            # end finished
      ...
      unless finished
        output_array << %w[Yes Yes]
        output_array << %w[No No]
        output_array << %w[Unsure Unsure]
    ...
    def migrate_quality_questions(efps, ef_id, qdf_arr, qrf_arr, ef_key_questions)
      ...
      qdf_arr.each do |qdf|
        ...
        name = Regexp.last_match(1) || qdf_title
      ...
      qrf_name = "Adjust Quality Rating (for Key Questions: #{(ef_key_questions.map do |ef_kq|
 get_srdrplus_key_question(ef_kq['key_question_id']).try(:ordering).try(:position).to_s end - ['']).join(', ')})"
    ...
    def migrate_multiple_choice_question(question, question_type, legacy_question, fields_of_legacy_question,
                                         data_points_of_legacy_question, table_root)
    ...
    def migrate_multi_row_question(question, question_type, legacy_question, _fields_of_legacy_question,
                                   data_points_of_legacy_question, table_root)
    ...
    def migrate_matrix_dropdown_question(question, legacy_question, data_points_of_legacy_question, table_root)
      ...
      (r_farr.select { |rf| rf['row_number'] != -1 } + r_farr.select { |rf| rf['row_number'] == -1 }).each do |rf|
        ...
        c_farr.each do |cf|
          ...
          data_points_of_legacy_question.select do |dp|
 dp['column_field_id'] == cf['id'] && dp['row_field_id'] == rf['id'] end.each do |dp|
            queue_data_point dp, qrc.question_row_column_fields.first
          end
        end
      end
    ...
    def migrate_type1_data(study_id, extraction)
      ...
      arms.each_with_index do |arm, ix|
        ...
        eefps_t1 = ExtractionsExtractionFormsProjectsSectionsType1.find_or_create_by \
          ...
          type1: t1
        if eefps_t1.ordering.nil? then Ordering.find_or_create_by(orderable: eefps_t1,
                                                                  position: ix + 1) else
                                                                                      eefps_t1.ordering.update(position: ix + 1) end
    ...
    def migrate_results_data(legacy_subgroup, legacy_outcome, eefpst1r)
      ...
      wac_arr.each do |wac|
        ...
        comp_arr.each do |comp|
          ...
          ComparisonsResultStatisticSection.find_or_create_by comparison: new_comparison,
    ...
    def migrate_study_data(study_id, extraction)
      get_data_point_queue_for_study(study_id).each do |_q_id, q_item|
        case q_item[:dp_type]
        when 'quality_rating'
    ...
    def migrate_quality_dimension_data(_study_id, extraction, q_item)
    ...
    def migrate_quality_rating_data(_study_id, extraction, q_item)
      ...
      data_points.each do |dp|
        ...
        Record.find_or_create_by! recordable: rating_eefps_qrcf, name: qrcqrco_id.to_s if qrcqrco_id.present?
      end
    end

    def migrate_question_data(_study_id, extraction, q_item)
      qrcf = q_item[:qrcf]
      ...
      if qrcf.question.extraction_forms_projects_section.extraction_forms_projects_section_option.by_type1
        ...
        adict.each do |eefpst1_id, dps|
          ...
          when @qrc_type_checkbox
            record_name = '[' + (dps.map do |dp|
 get_qrcqrco(qrc_id, dp['value']).to_s end - [nil, '', ' ']).join(', ') + ']'
            Record.find_or_create_by recordable: eefps_qrcf, name: record_name
            dps.each do |dp|
              ff = get_ff_by_qrcqrco get_qrcqrco(qrc_id, dp['value'])
              next unless ff.present? and dp['subquestion_value']
        ...
        when @qrc_type_checkbox
          record_name = '[' + (data_points.map do |dp|
 get_qrcqrco(qrc_id, dp['value']).to_s end - [nil, '', ' ']).join(', ') + ']'
          data_points.each do |dp|
            ff = get_ff_by_qrcqrco get_qrcqrco(qrc_id, dp['value'])
            next unless ff.present? and dp['subquestion_value']
    ...
    def migrate_primary_publication_as_citation(primary_publication_hash)
      ...
      Citation.new name: primary_publication_hash['title'] || '',
    ...
    def split_authors_string(_authors_string)
      # TODO
      []
    end
